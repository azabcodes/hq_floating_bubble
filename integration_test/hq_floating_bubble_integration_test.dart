// Integration tests for hq_floating_bubble plugin
//
// These tests simulate end-to-end workflows by mocking the native layer.
// Since hq_floating_bubble requires Android-specific features (SYSTEM_ALERT_WINDOW),
...
void main() {
  ...
  group('End-to-end workflow tests', () {
    ...
    test('Complete window creation workflow', () async {
      ...
      final config = HQFloatingWindowConfig(
        ...
        height: 100,
        draggable: true,
        gravity: HQFloatingGravityType.rightBottom,
      );

      // Step 3: Create and start window
      final window = await plugin.createWindow(
        'test-overlay',
        config,
        start: true,
      );
    ...
    test('HQFloatingWindow lifecycle workflow', () async {
      final plugin = HQFloatingService();
    ...
    test('HQFloatingWindow update workflow', () async {
      final plugin = HQFloatingService();
      ...
      final updateResult = await window?.update(
        HQFloatingWindowConfig(x: 50, y: 100, gravity: HQFloatingGravityType.center),
      );
    ...
    test('Data sharing workflow', () async {
      ...
      // Create window
      final config = HQFloatingWindowConfig(route: '/share-test');
    ...
    test('HQFloatingEvent registration workflow', () async {
      final plugin = HQFloatingService();

      // Create window
      final config = HQFloatingWindowConfig(route: '/event-test');
      ...
      window
          ...
          .on(HQFloatingEventType.WindowDragging, (w, data) {
            // Handle dragging
          });
    ...
    test('HQFloatingWindowConfig to HQFloatingWindow workflow using to()', () async {
      // Using the fluent API
      final window = HQFloatingWindowConfig(
        id: 'fluent-test',
        route: '/fluent',
        width: 150,
        height: 150,
    ...
    test('Multiple windows workflow', () async {
      ...
      final window1 = await plugin.createWindow(
        'window-1',
        HQFloatingWindowConfig(route: '/window1'),
      );
      final window2 = await plugin.createWindow(
        'window-2',
        HQFloatingWindowConfig(route: '/window2'),
      );
      final window3 = await plugin.createWindow(
        'window-3',
        HQFloatingWindowConfig(route: '/window3'),
      );
  ...
  group('Configuration combinations', () {
    test('Full-screen overlay config (night mode style)', () {
      ...
      expect(config.width, equals(HQFloatingWindowSize.matchParent));
      expect(config.height, equals(HQFloatingWindowSize.matchParent));
    ...
    test('Floating button config (assistive touch style)', () {
      ...
      expect(config.gravity, equals(HQFloatingGravityType.rightBottom));
    ...
    test('All gravity types should be distinct', () {
      final gravities = [
        HQFloatingGravityType.center,
        HQFloatingGravityType.centerTop,
        HQFloatingGravityType.centerBottom,
        HQFloatingGravityType.leftTop,
        HQFloatingGravityType.leftCenter,
        HQFloatingGravityType.leftBottom,
        HQFloatingGravityType.rightTop,
        HQFloatingGravityType.rightCenter,
        HQFloatingGravityType.rightBottom,
      ];
