...
void main() {
  ...
  group('HQFloatingSystemConfig', () {
    test('should create from map correctly', () {
      ...
      final config = HQFloatingSystemConfig.fromMap(map);
    ...
    test('should handle empty map', () {
      final config = HQFloatingSystemConfig.fromMap({});
    ...
    test('should create Size object only when both dimensions are present', () {
      ...
      final config1 = HQFloatingSystemConfig.fromMap({
        'screen': {'width': 100, 'height': 200},
      });
      expect(config1.screenSize, isNotNull);

      // Only width present
      final config2 = HQFloatingSystemConfig.fromMap({
        'screen': {'width': 100},
      });
      expect(config2.screenSize, isNull);

      // Only height present
      final config3 = HQFloatingSystemConfig.fromMap({
        'screen': {'height': 200},
      });
      expect(config3.screenSize, isNull);

      // Neither present
      final config4 = HQFloatingSystemConfig.fromMap({'screen': {}});
    ...
    test('should preserve exact values through toMap', () {
      ...
      final config = HQFloatingSystemConfig.fromMap(originalMap);
      ...
      expect(resultScreen['width'], equals(originalScreen['width']));
      expect(resultScreen['height'], equals(originalScreen['height']));
    });
  });
}
