...
class _AssistiveTouchState extends State<AssistiveTouch> {
  ...
  void initAsyncState() async {
    ...
    pannelWindow
        ?.on(HQFloatingEventType.WindowCreated, (window, data) {
          pannelReady = true;
          setState(() {});
        })
        .on(HQFloatingEventType.WindowPaused, (window, data) {
  ...
  void _onTap() {
    var w = HQFloatingWindow.of(context);
...
class _AssistiveButtonState extends State<AssistiveButton>
  ...
  Widget build(BuildContext context) {
    ...
    if (window == null) {
      ...
      window?.on(HQFloatingEventType.WindowDragStart, (window, data) => _onDragStart());
      window?.on(HQFloatingEventType.WindowDragging, (window, data) {
        var p = data as List<dynamic>;
        _onDragUpdate(p[0], p[1]);
      });
      window?.on(HQFloatingEventType.WindowDragEnd, (window, windowdata) => _onDragEnd());
    }
  ...
  void _updatePosition() {
    window?.update(HQFloatingWindowConfig(x: offset.dx.toInt(), y: offset.dy.toInt()));
  }
